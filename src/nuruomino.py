...
from search import Node, Problem, astar_search, InstrumentedProblem, breadth_first_graph_search, depth_first_graph_search
import copy
import time
import tracemalloc
...
class Nuruomino(Problem):
        ...
        for (i, j) in positions:
            for (ni, nj) in board.adjacent_positions(i, j):
                if (ni, nj) not in positions:
                    if board.get_value(ni, nj) == piece_letter:
                        return True
        return False
    ...
    def goal_test(self, state: NuruominoState):
        ...
        return (self._has_no_2x2_blocks(board) and self._all_regions_have_one_piece(board))
